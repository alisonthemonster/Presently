...
CircleCI can be set up with three separate pipeline configs:

- `.circleci/config.yml` for normal CI
- `.circleci/release.yml` for release builds
- `.circleci/promote.yml` for Play promotion only

If your CircleCI project uses the GitHub App pipeline model, add separate pipelines in CircleCI Project Settings and point them at those config paths. That keeps the trigger UI split so release runs only show `release_*` parameters and promote runs only show `promote_*` parameters.

The release pipeline is intended to run with a restricted CircleCI context named `presently-release`.

For the release pipeline, use `Trigger Pipeline` and pass:
...
For the promote pipeline, trigger a pipeline with:
...
- release pipeline with `release_bump: patch`, `release_play_track: internal`, and `release_commit_version: true`
- verify the internal release in Play
- promote pipeline to move the existing artifact from `internal` to `production`
